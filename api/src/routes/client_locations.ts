 ...
 *                     active: { type: boolean }
 */
router.get('/', asyncHandler(async (req, res) => { const { page, limit, offset } = getPagination(req); const pool = await getPool(); const r = await pool.request().input('offset', sql.Int, offset).input('limit', sql.Int, limit).query(`SELECT location_id, client_id, name, address, active, COUNT(*) OVER() AS total FROM app.client_locations ORDER BY location_id OFFSET @offset ROWS FETCH NEXT @limit ROWS ONLY`); const total = r.recordset[0]?.total ?? 0; const items = r.recordset.map((row:any)=>{ const { total: _t, ...rest } = row; return rest; }); listOk(res, items, { page, limit, total }); }));
/**
 * @openapi
 ...
 */
router.post('/', asyncHandler(async (req, res) => { const parsed = ClientLocationCreate.safeParse(req.body); if (!parsed.success) return badRequest(res, parsed.error.issues.map(i=>i.message).join('; ')); const { client_id, name, address = null, active = true } = parsed.data; const pool = await getPool(); const result = await pool.request().input('client_id', sql.Int, client_id).input('name', sql.NVarChar(200), name).input('address', sql.NVarChar(1000), address).input('active', sql.Bit, active ? 1 : 0).query(`INSERT INTO app.client_locations (client_id, name, address, active) OUTPUT INSERTED.location_id, INSERTED.client_id, INSERTED.name, INSERTED.address, INSERTED.active VALUES (@client_id, @name, @address, @active)`); ok(res, result.recordset[0], 201); }));
router.put('/:id', asyncHandler(async (req, res) => { const id = Number(req.params.id); if (!Number.isInteger(id) || id <= 0) return badRequest(res,'id must be a positive integer'); const parsed = ClientLocationUpdate.safeParse(req.body); if (!parsed.success) return badRequest(res, parsed.error.issues.map(i=>i.message).join('; ')); const data = parsed.data; const sets: string[] = []; const pool = await getPool(); const request = pool.request().input('id', sql.Int, id); if (data.name !== undefined) { sets.push('name=@name'); request.input('name', sql.NVarChar(200), data.name); } if (data.address !== undefined) { sets.push('address=@address'); request.input('address', sql.NVarChar(1000), data.address); } if (data.active !== undefined) { sets.push('active=@active'); request.input('active', sql.Bit, data.active ? 1 : 0); } if (!sets.length) return badRequest(res,'No fields to update'); const result = await request.query(`UPDATE app.client_locations SET ${sets.join(', ')} WHERE location_id=@id`); if (result.rowsAffected[0]===0) return notFound(res); const read = await pool.request().input('id', sql.Int, id).query(`SELECT location_id, client_id, name, address, active FROM app.client_locations WHERE location_id=@id`); ok(res, read.recordset[0]); }));

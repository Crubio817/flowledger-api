...
router.get('/', asyncHandler(async (req, res) => {
  ...
  const r = await pool.request().input('offset', sql.Int, offset).input('limit', sql.Int, limit).query(`SELECT client_id, tag_id FROM app.client_tag_map ORDER BY client_id OFFSET @offset ROWS FETCH NEXT @limit ROWS ONLY`);
...
router.delete('/', asyncHandler(async (req, res) => {
  ...
  if (Number.isNaN(clientId) || Number.isNaN(tagId)) return badRequest(res, 'client_id and tag_id required');
  const pool = await getPool();
